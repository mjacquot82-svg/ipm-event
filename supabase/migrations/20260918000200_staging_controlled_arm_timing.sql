-- Align the staging controlled arm window with the UI and discovery metadata.
...
begin
  ...
  -- Valid arm interval is [event - lead, event - (lead - 1 minute)).
  if item_record.starts_at > p_now + make_interval(mins => fixture_record.test_lead_minutes)
     or item_record.starts_at <= p_now + make_interval(mins => fixture_record.test_lead_minutes - 1)
