...
create table public.notification_reconciliation_project (
 ...
 -- Private, environment-local pilot reference. NULL excludes every registration.
 pilot_registration_id uuid references public.notification_installations(id),
 window_start timestamptz not null default now(), checks integer not null default 0,
...
begin
 ...
 if reg.id is null or project.pilot_registration_id is null or reg.id is distinct from project.pilot_registration_id then
   return jsonb_build_object('status','INELIGIBLE','outcome','NOT_PILOT','pilot_eligible',false);
 end if;
 if action='eligibility' then
   return jsonb_build_object('pilot_eligible',true,'observation_enabled',project.enabled,'repair_enabled',project.repair_enabled);
 end if;
