-- Additive population-mode controls. Defaults preserve the verified pilot.
alter table public.notification_reconciliation_project
  add column if not exists mode text not null default 'PILOT'
    check (mode in ('PILOT','POPULATION_OBSERVE','POPULATION_REPAIR_STAGED')),
...
-- Rollback after disabling callers: restore the previous function and drop the added columns.
-- alter table public.notification_reconciliation_project drop column repair_cohort_percent, drop column mode;
