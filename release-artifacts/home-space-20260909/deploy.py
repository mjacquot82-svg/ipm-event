...
BEFORE='6aa156252cf3f0faf2aa6a74' if target=='staging' else '6aa156c72aa93a677211488a'
branch='fix/home-space-staging-20260909' if target=='staging' else 'fix/home-space-production-20260909'
work=Path('/workspaces/ipm-event/.worktrees/home-space-staging' if target=='staging' else '/workspaces/ipm-event/.worktrees/home-space-production')
...
if mode=='draft':
 ...
 assert len(next(dist.glob('_expo/static/js/web/*.js')).read_bytes())>2000000,'Incomplete app export'
 for p in paths:
 ...
 d=req('/sites/'+SITE+'/deploys','POST',dict(files=files,draft=True,title='Compact Home notification invitation '+source,branch=branch));print(save('draft',d),flush=True)
