...
process.on('uncaughtException',async error=>{const diagnostics=[];for(const c of clients){const workers=[];for(const w of c.context.serviceWorkers()){try{workers.push(await Promise.race([w.evaluate(async()=>({version:typeof IPM_OFFLINE_VERSION==='string'?IPM_OFFLINE_VERSION:null,cache:await Promise.all((await caches.keys()).map(async key=>{const cache=await caches.open(key);const html=await cache.match('/index.html');return{key,entry:html?(await html.text()).match(/entry-[a-f0-9]+\\.js/)?.[0]:null,requests:(await cache.keys()).map(r=>new URL(r.url).pathname)}}))})),pause(2000).then(()=>({timeout:true}))]));}catch(e){workers.push({error:String(e)})}}diagnostics.push({name:c.name,workers});}writeFileSync(A+'failure-diagnostics.json',JSON.stringify({error:String(error),diagnostics},null,2));await browser.close();process.exit(1);});
const trace=(name,data)=>appendFileSync(A+'network.jsonl',JSON.stringify({at:Date.now(),name,...data})+'\n');
const scenarios=preview?['browser320','installed360','desktop']:['slow'];
...
for(const phase of ['B','C']){
 ...
 for(const c of clients){
  ...
  if(c.name==='slow'||c.name==='failed-asset'){
   c.fault=c.name==='slow'?'slow':'asset';if(!c.page.isClosed())await c.page.close();c.page=await c.context.newPage();const started=Date.now();await c.page.goto(base+'/itinerary');await c.page.getByRole('button',{name:'Remove Upgrade fixture event from itinerary',exact:true}).waitFor();fallback={...await snapshot(c.page),elapsedMs:Date.now()-started};writeFileSync(A+phase+'-'+c.name+'-fallback.json',JSON.stringify(fallback,null,2));assert.equal(fallback.entry,c.oldEntry,'fault must preserve old usable app');assert(fallback.elapsedMs<12000,'fallback unbounded');assert(fallback.elapsedMs<8000,'fallback must have meaningful margin');c.fault=null;
  }
