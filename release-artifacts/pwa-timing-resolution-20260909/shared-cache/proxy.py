...
class Handler(BaseHTTPRequestHandler):
 ...
 def do_GET(self):
  ...
  if path=='/webpushr-sw.js':
   if cfg.get('fault')=='slow-worker-check':time.sleep(20)
   if cfg.get('fault') in ['worker-check','slow-worker-check']:self.send_error(503);return
   if cfg.get('fault')=='activation-race':time.sleep(2)
  ...
  with lock:
   with (A/'proxy-network.jsonl').open('a') as f:f.write(json.dumps({'at':time.time(),'phase':phase,'path':self.path.split('?')[0],'event':'response','fault':cfg.get('fault')})+'\n')
