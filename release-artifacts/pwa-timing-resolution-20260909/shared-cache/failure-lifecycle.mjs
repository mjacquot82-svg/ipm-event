...
state('C');await c.context.setOffline(true);await c.context.setOffline(false);
...
state('C');for(let i=0;i<4;i++){const current=await fresh(ident('C'));const registrations=await c.page.evaluate(async()=> (await navigator.serviceWorker.getRegistrations()).length);assert.equal(registrations,1);assert(current.cache.length<=2);results.push({name:'already-current-'+i,current,registrations});}
let reloads=0;c.page.on('framenavigated',f=>{if(f===c.page.mainFrame())reloads++;});await pause(7000);assert.equal(reloads,0);
writeFileSync(A+'proxy-results.json',JSON.stringify({passed:true,results},null,2));await browser.close();
