...
state('B','worker-check');const failedCheck=await fresh(ident('B'));await c.context.setOffline(true);await c.context.setOffline(false);await pause(5000);writeFileSync(A+'worker-call-observation.json',JSON.stringify(await c.page.evaluate(()=>window.__workerCalls),null,2));
const checks=readFileSync(A+'proxy-network.jsonl','utf8').trim().split('\n').map(x=>JSON.parse(x)).filter(x=>x.path==='/webpushr-sw.js'&&x.fault==='worker-check');assert(checks.length>0,'must observe real browser worker HTTP check failing');
results.push({name:'worker-check-503',failedCheck,failedScriptRequests:checks.length});console.log('PASS worker-check');
state('B');await c.context.setOffline(true);await c.context.setOffline(false);await pause(3000);await fresh(ident('B'));
c.offline=true;await c.context.setOffline(true);const offlineB=await fresh(ident('B'));c.offline=false;await c.context.setOffline(false);results.push({name:'recovery-offline-B',offlineB});
state('C','slow-worker-check');const slowCheck=await fresh(ident('C'));await c.context.setOffline(true);await c.context.setOffline(false);await pause(22000);results.push({name:'slow-worker-check',slowCheck});
state('C','activation-race');await c.context.setOffline(true);await c.context.setOffline(false);
const race=await fresh(ident('C'));await pause(3000);c.offline=true;await c.context.setOffline(true);const raceOffline=await fresh(ident('C'));c.offline=false;await c.context.setOffline(false);results.push({name:'activation-overlap',race,raceOffline});console.log('PASS activation-overlap');
for(const fault of ['slow','server']){state('C',fault);const fallback=await fresh(ident('C'));results.push({name:fault,fallback});console.log('PASS',fault,fallback.elapsedMs);}
state('C');for(let i=0;i<4;i++){const current=await fresh(ident('C'));const registrations=await c.page.evaluate(async()=> (await navigator.serviceWorker.getRegistrations()).length);assert.equal(registrations,1);assert(current.cache.length<=2);const calls=await c.page.evaluate(()=>window.__workerCalls);assert(calls.filter(x=>x.key==='register').length<=1);assert(calls.filter(x=>x.key==='update').length<=2);results.push({name:'already-current-'+i,current,registrations,calls});}
let reloads=0;c.page.on('framenavigated',f=>{if(f===c.page.mainFrame())reloads++;});await pause(7000);assert.equal(reloads,0);
writeFileSync(A+'extended-results.json',JSON.stringify({passed:true,results},null,2));await browser.close();
