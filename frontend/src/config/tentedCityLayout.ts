import { TENTED_CITY_MAP } from './tentedCityTypes';

export const TENTED_CITY_IMAGE_ASPECT =
  TENTED_CITY_MAP.imageWidth / TENTED_CITY_MAP.imageHeight;

/** Letterbox the official artwork into a viewport. Never returns 0. */
export function tentedCityFittedSize(vw: number, vh: number) {
  const width = Math.max(vw, 1);
  const height = Math.max(vh, 1);
  const viewAspect = width / height;
  if (viewAspect > TENTED_CITY_IMAGE_ASPECT) {
    return { width: height * TENTED_CITY_IMAGE_ASPECT, height };
  }
  return { width, height: width / TENTED_CITY_IMAGE_ASPECT };
}
 ...
 */
export function tentedCityPaintViewport(
...
export function tentedCityLayerLayout(viewport: { width: number; height: number }) {
