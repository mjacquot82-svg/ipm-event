...
export function ParadeRouteOverlay({ routeId }: { routeId: ParadeRouteId | null }) {
  ...
  const svg = `<svg xmlns="http://www.w3.org/2000/svg" viewBox="${PARADE_VIEWBOX}">
    ...
    ${route.labels.map(({text, lines=[text], at:[x,y], angle=0, fontSize=7}) => { const lineHeight = fontSize * 1.15; const maxChars = Math.max(...lines.map(line => line.length)); const width = maxChars * fontSize * 0.62 + 8; const height = lines.length * lineHeight + 4; const firstY = -((lines.length - 1) * lineHeight) / 2; const lineMarkup = lines.map((line, index) => `<tspan x="${x}" dy="${index === 0 ? firstY : lineHeight}">${line}</tspan>`).join(''); return `<g transform="rotate(${angle} ${x} ${y})"><rect x="${x - width / 2}" y="${y - height / 2}" width="${width}" height="${height}" rx="${height / 2}" fill="${BLUE}"/><text x="${x}" y="${y}" text-anchor="middle" dominant-baseline="middle" font-family="sans-serif" font-size="${fontSize}" font-weight="700" fill="white" stroke="#003B5C" stroke-width="0.9" stroke-linejoin="round" paint-order="stroke" letter-spacing="0">${lineMarkup}</text></g>`; }).join('')}
  </svg>`;
