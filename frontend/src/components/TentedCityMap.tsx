...
import {
  clampTranslation, DOUBLE_TAP_SCALE, flyToRect, rubberBandTranslation, translationBounds, zoomAroundFocal,
...
export default function TentedCityMap({
  ...
  const pinch = Gesture.Pinch()
    ...
    .onUpdate((e) => {
      ...
      scale.value = next.scale; tx.value = soft.tx; ty.value = soft.ty;
    })
    .onEnd(() => {
      const layout = { viewportW: viewW.value, viewportH: viewH.value, mapW: mapW.value, mapH: mapH.value, left: originX.value, top: originY.value };
      if (scale.value <= 1.02) {
  ...
  const pan = Gesture.Pan().minPointers(1).maxPointers(1)
    .onBegin(() => { cancelAnimation(tx); cancelAnimation(ty); startX.value = tx.value; startY.value = ty.value; })
    .onUpdate((e) => {
      const layout = { viewportW: viewW.value, viewportH: viewH.value, mapW: mapW.value, mapH: mapH.value, left: originX.value, top: originY.value };
  ...
  return (
    <View style={styles.root} collapsable={false}>
      <View
        ref={viewportRef}
        style={[styles.viewport, Platform.OS === 'web' ? ({ touchAction: 'none', overscrollBehavior: 'none', userSelect: 'none' } as object) : null]}
        onLayout={onLayout}
        collapsable={false}
      >
        <GestureDetector gesture={composed}>
          <Animated.View style={styles.gestureRoot} collapsable={false}>
