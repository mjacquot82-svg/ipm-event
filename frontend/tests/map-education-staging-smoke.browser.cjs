...
(async()=>{const b=await chromium.launch({args:['--no-sandbox']});try{
 ...
 const p=await c.newPage(),errors=[];p.on('pageerror',e=>errors.push(e.message));await p.goto(origin+'/map');const card=p.getByTestId('map-education-card');for(let i=0;i<3;i++){await card.getByText(`${i+1} of 3`,{exact:true}).waitFor();await card.getByRole('button',{name:i===2?'Got it':'Next',exact:true}).click();}
 await p.getByRole('button',{name:'Help, replay Maps tour'}).click();await card.getByText('Find parking',{exact:true}).waitFor();await p.keyboard.press('Escape');await p.getByTestId('grounds-view-parking').click();await p.getByTestId('grounds-parking-1').click();await p.getByLabel('Close parking information').click();await p.getByTestId('grounds-fit-reset').click();
 await p.setViewportSize({width:1440,height:1000});await p.getByRole('button',{name:'Help, replay Maps tour'}).click();await card.getByRole('button',{name:'Skip Maps tour'}).click();await p.getByTestId('map-mode-rv').click();await p.getByTestId('rv-site-search').fill('M27');await p.getByText('RV Site M27',{exact:true}).click();await p.getByTestId('rv-site-highlight').waitFor();
