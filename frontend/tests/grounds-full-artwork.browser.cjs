...
const out = process.env.IPM_GROUNDS_OUTPUT || path.resolve(__dirname, '../../.artifacts/grounds-full-artwork');
...
(async () => {
 ...
 try {
  for (const [width, height] of [[320,568],[360,800],[390,844],[412,915],[768,1024],[1440,900]]) {
   ...
   await page.getByTestId('map-mode-grounds').waitFor();
   ...
   await page.getByTestId('map-mode-grounds').click();
