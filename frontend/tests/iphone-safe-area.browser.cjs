  ...
  try {
   for (const width of (process.env.IPM_WIDTHS ? process.env.IPM_WIDTHS.split(',').map(Number) : [320,375,390,393,430,360,412,1440])) {
    ...
    for (const inset of insets) {
     ...
     await page.route('https://ipm-backend-eoiw.onrender.com/**',async r=>{if(r.request().method()!=='GET')return r.abort();const response=await r.fetch();await r.fulfill({response,headers:{...response.headers(),'access-control-allow-origin':'*'}});});
     ...
     if(width===393&&inset===59)await page.screenshot({path:`../diagnostics/iphone-safe-area/${engine.name()}-393-59.png`});
     await context.close();
    }
   }
