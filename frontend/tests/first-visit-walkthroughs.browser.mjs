...
try {for(const width of (process.env.IPM_TEST_WIDTHS ? JSON.parse(process.env.IPM_TEST_WIDTHS) : [320,768,1440])) {
 ...
 await p.goto(base+'/vendors');await tip.getByText('Find this vendor',{exact:true}).waitFor();await assertClickCue(p,'vendor-education-open-map');assert.match(await tip.innerText(),/Tap Find on Map/);assert.equal(await tip.getByRole('button',{name:'Got it',exact:true}).count(),0);await p.screenshot({path:`${out}/${width}-vendors-auto.png`});await p.getByTestId('vendor-education-open-map').click();await tip.getByText('Vendor location',{exact:true}).waitFor();await p.getByTestId('vendor-booth-highlight').first().waitFor();await tip.getByRole('button',{name:'Got it',exact:true}).click();
 ...
 // Production ignores the former staging preview query.
 await p.evaluate(()=>localStorage.setItem('walkthrough-unrelated-state-proof','keep-me'));
 for(const section of ['schedule','vendors','map']){
  await p.goto(base+'/'+section+'?previewWalkthrough=1');await visibleHelp(section);await noAutomatic();
 }
 ...
 console.log(`PASS ${width}: automatic Schedule → Vendors → Maps, independent completion, no repeat, visible replay, production preview query is inert`);await c.close();
}
