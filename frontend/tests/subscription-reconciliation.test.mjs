...
test('coordination uses Web Locks, bounded retries and strict production gate',()=>{
 ...
 assert.match(wrapper,/location.origin==='https:\/\/theipm.ca'/);
 assert.doesNotMatch(wrapper,/\.subscribe\(|\.unsubscribe\(|\.register\(|\.clear\(|\.init\(|subscribeToNotifications\(/);
});

test('pilot recovery bypasses legacy enrollment; nonpilot baseline stays separate',()=>{
 const service=readFileSync(new URL('../src/services/notificationRegistration.web.ts',import.meta.url),'utf8');
 const guarded=service.slice(service.indexOf('export async function ensureNotificationRegistration'));
 assert.ok(guarded.indexOf('isProductionPilot()') < guarded.indexOf('runNotificationRegistrationAttempt'));
 assert.match(guarded,/if \(pilot\)/);
