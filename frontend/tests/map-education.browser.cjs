...
(async()=>{
 ...
 try {
  ...
  await p.evaluate(ks=>{localStorage.setItem(ks[1],'true');localStorage.setItem(ks[2],'true');},keys);await p.getByRole('button',{name:'Map Help, replay Maps tour'}).click();await card(p).waitFor();await card(p).getByRole('button',{name:'Skip tutorial'}).click();assert.deepEqual(await p.evaluate(ks=>ks.map(k=>localStorage.getItem(k)),keys),['true','true','true']);
  ...
  const skipContext=await setup(b),skip=await skipContext.newPage();await skip.goto(origin+'/map');await card(skip).getByRole('button',{name:'Skip tutorial'}).click();await skip.reload();await skip.getByRole('button',{name:'Map Help, replay Maps tour'}).waitFor();await skip.waitForTimeout(900);assert.equal(await card(skip).count(),0);await skipContext.close();
