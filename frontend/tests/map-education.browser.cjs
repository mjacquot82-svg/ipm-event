...
const targets=['map-mode-grounds','search','parade-routes-control','tented-map-reset','map-mode-rv'];
...
async function step(p,index,width) {
 ...
 if(index===0) assert.equal(await p.getByTestId('grounds-view-general').getAttribute('aria-selected'),'true');
 assert.equal(await p.getByTestId(index===0?'map-mode-grounds':'map-mode-tented').evaluate(e=>getComputedStyle(e).backgroundColor),'rgb(255, 255, 255)');
...
(async()=>{
 ...
 try {
  ...
  await p.getByTestId('map-mode-rv').click();await p.getByRole('button',{name:'Help, replay Maps tour'}).click();await card(p).getByText(/Parking areas are shown/).waitFor();await p.keyboard.press('Escape');assert.equal(await p.getByTestId('map-mode-rv').evaluate(e=>getComputedStyle(e).backgroundColor),'rgb(255, 255, 255)');
