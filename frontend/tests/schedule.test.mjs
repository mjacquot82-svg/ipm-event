...
test('release schedule data excludes known staging T-30 fixture signatures', () => {
  ...
  for (const marker of forbiddenMarkers) {
    assert.equal(serialized.includes(marker), false, `staging fixture leaked into release data: ${marker}`);
  }
});
...
test('mobile category button truncates long names while accessibility retains the full name', () => {
