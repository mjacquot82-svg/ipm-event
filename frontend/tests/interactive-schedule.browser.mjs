...
try {for(const width of [320,768,1440]) {
 ...
 async function followInteractiveFlow(label,event=schedule.events[0]) {
  ...
  assert.doesNotMatch(await p.getByTestId('map-selection-card').innerText(),/10:00|10:15/);await tip.getByText('Find this event',{exact:true}).waitFor();await tip.getByRole('button',{name:'Got it',exact:true}).click();assert.equal(await tip.count(),0);
 }
