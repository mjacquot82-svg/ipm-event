...
try {for(const width of [320,768,1440]) {
 ...
 async function followInteractiveFlow(label) {
  ...
  await p.getByText(schedule.events[0].title,{exact:true}).first().scrollIntoViewIfNeeded();
  ...
  assert.equal(await p.getByTestId('map-selection-title').innerText(),schedule.events[0].title);
 ...
 await p.getByRole('button',{name:'Schedule Help',exact:true}).click();await followInteractiveFlow('replay');
