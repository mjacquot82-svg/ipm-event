...
const filteredEvent={...schedule.events[0],id:'33333333-4444-4555-8666-777777777777',title:'Fixture filtered event'};schedule.events.push(filteredEvent,...Array.from({length:6},(_,i)=>({...schedule.events[0],id:`00000000-0000-4000-8000-00000000000${i}`,title:`Extra scrolling event ${i}`})));
...
try {for(const width of [320,768,1440]) {
 ...
 async function followInteractiveFlow(label,event=schedule.events[0]) {
  ...
  await assertClickCue(p,'schedule-education-open-event');
  const cueBox=await p.getByTestId('tutorial-click-cue').boundingBox();await p.mouse.click(cueBox.x+cueBox.width/2,cueBox.y+cueBox.height/2);await tip.getByText('Tap an event',{exact:true}).waitFor();
  if(label==='fresh') {
  const before=(await p.getByTestId('schedule-education-open-event').boundingBox()).y;
  await p.getByText(event.title,{exact:true}).first().evaluate(el=>{for(let node=el.parentElement;node;node=node.parentElement){if(node.scrollHeight>node.clientHeight&&/auto|scroll/.test(getComputedStyle(node).overflowY)){node.scrollTop+=16;return;}}});
  await p.waitForTimeout(600);
  assert.notEqual((await p.getByTestId('schedule-education-open-event').boundingBox()).y,before,'target moved with scrolling');
  }
  await assertClickCue(p,'schedule-education-open-event');
  ...
  assert.doesNotMatch(await p.getByTestId('map-selection-card').innerText(),/10:00|10:15/);await tip.getByText('Find this event',{exact:true}).waitFor();await assertNoClickCue(p);await tip.getByRole('button',{name:'Got it',exact:true}).click();assert.equal(await tip.count(),0);await assertNoClickCue(p);
 }
