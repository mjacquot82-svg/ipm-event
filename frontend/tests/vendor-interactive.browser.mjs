...
try {for(const [width,height] of (process.env.IPM_TEST_VIEWPORTS ? JSON.parse(process.env.IPM_TEST_VIEWPORTS) : [[320,568],[390,844],[768,1024],[1440,900]])) {
 ...
 async function flow(label,manual=true){
  ...
  const vendor=await p.getByTestId('vendor-find-on-map').evaluateAll((buttons,r)=>{
   ...
   return button?.parentElement?.parentElement?.firstElementChild?.textContent;
  },rect);
  ...
  const url=new URL(p.url()),location=url.searchParams.get('vendorName') || url.searchParams.get('location');assert.equal(url.searchParams.get('source'),'vendors');assert.equal(url.searchParams.get('mapType'),'tented');assert(location);assert.equal(url.searchParams.get('vendorName') || location,url.searchParams.get('vendorName') ? vendor : resolveVendorMapQuery(vendor).query,'map destination matches the highlighted vendor identity or approved crosswalk');
