...
test('iPhone Chrome explains the Safari handoff and installation steps', () => { const g = getInstallGuidance(detect(iphoneChrome)); const copy = JSON.stringify(g); assert.match(g.intro, /keep using IPM in Chrome/i); assert.match(copy, /Open this page in Safari/); assert.match(copy, /Share button/); assert.match(copy, /Add to Home Screen/); });
...
test('current runtime state wins over legacy installed and entry-completed flags', () => {
