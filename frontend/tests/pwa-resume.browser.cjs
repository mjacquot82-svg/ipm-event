...
let version='legacy';
const baseline = require('node:child_process').execFileSync('git',['show','e93ead504d8734a0b35abce97bce59a36988d572:frontend/public/webpushr-sw.js'],{encoding:'utf8'});
...
(async()=>{
 ...
 version='A';await page.reload();await page.getByRole('heading',{name:'Version A',exact:true}).waitFor();await page.waitForFunction(()=>window.started);
 ...
 console.log('PASS: real production-worker bootstrap legacy→A and A→B explicit refresh exactly once; same release no-op; offline B; cold online C; storage/permission intact; no runtime errors. Provider stubbed.');
