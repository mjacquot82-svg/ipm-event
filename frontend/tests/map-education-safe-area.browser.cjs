...
(async()=>{for(const engine of (process.env.IPM_BROWSER==='webkit'?[webkit]:process.env.IPM_BROWSER==='chromium'?[chromium]:[chromium,webkit])){const b=await engine.launch({args:engine===chromium?['--no-sandbox']:[]});try{
 ...
 for(let i=0;i<3;i++){await card.waitFor();await p.waitForTimeout(300);const r=await card.boundingBox();assert.ok(r.x>=0&&r.x+r.width<=width&&r.y>=inset&&r.y+r.height<=p.viewportSize().height-(phone?34:0));const button=card.getByRole('button',{name:i===2?'Got it':'Next',exact:true});await button.click();}
 for(const mode of ['grounds','tented','rv']){await p.getByTestId('map-mode-'+mode).click();await p.waitForTimeout(250);const help=p.getByRole('button',{name:'Help, replay Maps tour'}),h=await help.boundingBox();assert.ok(h.x>=0&&h.x+h.width<=width&&h.y>=inset);assert.ok(await help.evaluate(e=>{const r=e.getBoundingClientRect(),hit=document.elementFromPoint(r.x+r.width/2,r.y+r.height/2);return e===hit||e.contains(hit)}),'Help hit target');
 ...
 console.log('PASS '+engine.name()+' 15 widths, all three steps, 59px top/34px bottom safe areas, short phone, Help hit targets and Fit/selector separation across all three maps');
