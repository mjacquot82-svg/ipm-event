...
try{
 ...
 await p.goto(base+'/schedule');await p.getByText('Plan your day',{exact:true}).waitFor();await p.getByRole('button',{name:'Got it, close Plan your day introduction'}).click();await p.getByText(schedule.events[0].title,{exact:true}).first().scrollIntoViewIfNeeded();await tip.getByText('View event details',{exact:true}).waitFor();await p.getByTestId('schedule-education-open-event').click();await p.getByTestId('schedule-find-on-map').scrollIntoViewIfNeeded();await tip.getByText('Find this event',{exact:true}).waitFor();await p.getByTestId('schedule-education-open-map').click();await p.getByTestId('map-selection-title').waitFor();
