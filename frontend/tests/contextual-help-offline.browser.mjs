...
try{
 ...
 await p.goto(base+'/vendors');await p.getByTestId('vendor-find-on-map').scrollIntoViewIfNeeded();await tip.getByText('Find this vendor',{exact:true}).waitFor();await tip.getByRole('button',{name:'Got it',exact:true}).click();
