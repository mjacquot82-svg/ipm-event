...
(async()=>{await new Promise(r=>server.listen(0,'127.0.0.1',r));const b=await chromium.launch({args:['--no-sandbox']});try{
 ...
 console.log('PASS: generated staging shell installs, reloads Maps offline, switches Camping, and resolves manual M27. Push provider stubbed; no remote writes.');
