...
const base=process.env.IPM_TEST_URL||'http://localhost:8099';
const browser=await chromium.launch({executablePath:process.env.CHROMIUM_PATH,args:['--no-sandbox']});
for(const width of [320,390,1440])for(const scenario of ['success','denied','unavailable','network','provider','rate']){
 ...
 const p=await c.newPage();const urls=[],logs=[];let calls=0;
 ...
 if(scenario==='success'){await p.screenshot({path:`/tmp/ipm-emergency-private-${width}.png`,fullPage:true});await p.reload();await action.waitFor();assert.equal(await p.getByText('Read this to 911',{exact:true}).count(),0);}
 console.log('PASS',width,scenario,'body-only POST, safe errors, no URL/log/storage leak, no automatic lookup');await c.close();
}
