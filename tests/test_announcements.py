...
class FakeDeliveryService:
    ...
    async def mark_sent(self, delivery_id, provider_delivery_id):
        ...
        row.update(status="sent", provider_delivery_id=provider_delivery_id,
                   provider_accepted_at=datetime.now(timezone.utc),
...
def test_test_send_uses_only_configured_subscribers(monkeypatch):
    ...
    assert deliveries.rows[0]["target_url"].startswith("https://staging.theipm.ca/announcements/announcement-1?notification_ref=")
