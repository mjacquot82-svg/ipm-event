...
def test_identity_event_and_capability_isolation_no_metadata_created():
 for kw in [{'capability_hash':'x'*64},{'installation_id':'z'*40},{'event_slug':'production'}]:
  assert rpc(**kw)['status']=='IDENTITY_UNRESOLVED'
...
def test_project_load_1000_attendee_launches_bounded_to_120_cycles():
 ...
 assert sum(r['status']=='COMPARING' for r in results)==120
 assert sum(r.get('outcome')=='RATE_LIMIT' for r in results)==880
