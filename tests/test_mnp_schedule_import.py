...
class MnpScheduleImportTests(unittest.TestCase):
    ...
    def test_multiline_workbook_content_is_preserved_exactly(self):
        ...
        self.assertEqual("Cottage Springs", events["2026-09-23-quality-homes-g27"]["title"])
        self.assertEqual("Oil Sampling", events["2026-09-24-harleys-i27"]["title"])
    ...
    def test_exact_admin_owned_heather_stark_row_is_an_external_exception(self):
        desired = next(row for row in desired_rows(load_manifest(), "event-uuid")
            if row["external_id"] == HEATHER_STARK_EXCEPTION["external_id"])
        existing = {
            **desired,
            **HEATHER_STARK_EXCEPTION,
            "event_id": "event-uuid",
            "title": "Furniture Refresh",
            "description": "Willow Home",
        }
        result = classify([existing], [desired])
        self.assertEqual(1, len(result["EXTERNAL_EXCEPTION"]))
        self.assertFalse(result["INSERT"] or result["UPDATE"] or result["CONFLICT"])

        for field, value in {
            "id": "wrong-uuid",
            "source": "another_source",
            "starts_at": "2026-09-23T14:00:00-04:00",
            "location_name": "Wrong stage",
            "category": "Wrong category",
            "status": "draft",
            "sort_order": 999,
        }.items():
            changed = {**existing, field: value}
            self.assertEqual(1, len(classify([changed], [desired])["CONFLICT"]), field)
