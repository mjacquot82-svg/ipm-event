"""Generate the one-occurrence Angie resolution patch; never changes a schema or calls providers."""
import json
...
SOURCE = 'artisan_angie_resolution_20260919'
BASELINE_HASH = '3ae840a54cf69eb0d50cdc7896efe71c'
MANIFEST = Path(__file__).parent / 'import_manifests/artisan_angie_resolution_2026.json'
...
def desired_rows():
    ...
    assert len(rows) == len({r['id'] for r in rows}) == 1
    return rows
...
BEGIN
  ...
  IF (SELECT count(*) FROM public.schedule_items WHERE source='{SOURCE}') <> 1
